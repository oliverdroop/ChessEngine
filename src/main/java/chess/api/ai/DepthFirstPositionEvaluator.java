...
public class DepthFirstPositionEvaluator {
    ...
    static ConfigurationScorePair getBestConfigurationScorePairRecursively(PieceConfiguration pieceConfiguration, int depth, boolean isInitialDepth) {
        final int currentDiff = pieceConfiguration.getDrawResult(pieceConfiguration.getValueDifferential(), isInitialDepth).score();
        ...
        for (int i = 0; i < onwardConfigurationCount; i++) {
            ...
            int nextDiff = onwardConfiguration.getDrawResult(onwardConfiguration.getValueDifferential(), isInitialDepth).score();
